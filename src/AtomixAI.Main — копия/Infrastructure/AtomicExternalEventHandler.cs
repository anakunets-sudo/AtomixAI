...
namespace AtomixAI.Main.Infrastructure
{
    public class AtomicExternalEventHandler : IExternalEventHandler
    {
        ...
        public void Execute(UIApplication app)
        {
            ...
            TransactionManager.TransactionFactory = (name) =>
                new RevitTransactionHandler(iuDoc, name);
            ...
            while (CommandQueue.Count > 0)
            {
                ...
                if (!result.Success)
                {
                    MessageBox.Show(
                        ...
                        $"Данные: {jsonArgs}" , "AtomixAI Error");

                    MessageBox.Show( result.Message, "Критическая ошибка");
                }
                else
                {
